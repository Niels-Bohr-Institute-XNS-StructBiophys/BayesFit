...
c
     ...
     *sd(500),x(500),y(500),smear(20,20),w(20)
...
cc      common /diff2/diff2(20)
      ...
      call getarg(1,aname)
      istart=0
      open(unit=77,file='Qcheck.d',status='unknown')
      open(unit=68,file='global_par.d',status='unknown')
      open(unit=16,file='errors.d',status='unknown')
      write(68,*)
      close(68)
...
c     read from the inputfile (8)
      read(8,3)fname
      read(8,3)model
      read(8,*)alphamin,alphamax,itot,nite,maxit
      close(8)
c     generate outputfilenames
      f2name='fit_'//fname
      f3name='output_'//fname
      f4name='start_'//fname
      f5name='parameters_'//fname
      open(unit=15,file=f5name,status='unknown')
      write(15,*)'# alpha     evidence   chi^2_r      N_g      alpha*S
     -  sum        chi^2     Iterations  mtot   Parameters --> '
      write(16,*)'# alpha     evidence   chi^2_r      N_g      alpha*S
     -  sum        chi^2     Iterations  mtot   Parameters --> '
      itot=itot-1
      j1=0
  10  continue
...
c
 ...
 980  continue
      ...
      write(6,32)'Output fit I(q)  :  ',f2name
      write(6,32)'Output parameters:  ',f5name
   31 format(x,i2,x,a,x,a)
   32 format(x,a,x,a)
      write(6,*)
      open(unit=9,file=f2name,status='unknown')
   22 continue
 ...
 125  read(10,*,end=111,err=110)x(i),y(i),sd(i)
  ...
  605 chi2=0
      ...
      write(9,*)'#  fit for alpha = ',alf,' below: '
      do 1000 i=1,mtot
      call funcs2(x(i),fit,ymod,dyda,ma)
      write(21,*)x(i),ymod,0
      write(9,*)x(i),ymod,0
...
c
      ...
      write(6,987)
     -'Chi^2_r, -log(Evidence) Ng = ',sumchi,ev,ng2
 987  format(1x,a,f8.3,3x,f10.3,3x,f8.3)
      write(15,901)alf,ev,sumchi,ng2,s,sum,sumchi*mtot,
     - kk-1,mtot,(fit(k)*sqrt(sigm2(k)),k=1,ma)
      write(16,901)alf,ev,sumchi,ng2,s,sum,sumchi*mtot,
     - kk-1,mtot,(sqrt(covar(k,k))*sqrt(sigm2(k)),k=1,ma)
 
c
c check the shape of the curves for Q and the Occam factor as a function of alpha
c  
      write(77,*)alf,Q,0.5*sum,0.5*sumchi*mtot,s,log(alf),s/alf
...
c******************************************************************************  
  ...
  5   continue
      ...
      glob(12)="   rho_p    "
      glob(13)="   rho_c    "
      open(unit=68,file='global_par.d',status='old',access='append')
      write(68,900)(glob(k),k=1,13)
  900 format(13a)
      endif
